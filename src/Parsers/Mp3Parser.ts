...
export class Mp3Parser implements IAudioParser {
  ...
  analyze(buffer: Buffer, opts: { fileSize?: number } = {}): AudioFileInfo {
    ...
    while (offset + 4 <= offsets.audioEnd) {
      ...
      if (channels === undefined) {
        ...
        channels = channelMode === 3 ? 1 : 2; // 11 = mono, others = stereo
      }
  ...
  private calculateFrameLength(frame: ParsedFrameHeader): number {
    if (frame.layer === 1) {
  ...
  private syncSafeInteger(bytes: Buffer): number {
  ...
  private parseFrameHeader(header: number): ParsedFrameHeader | null {
  ...
  private decodeVersion(bits: number): 1 | 2 | 25 | null {
    switch (bits) {
      case 0b11:
        return 1;
      case 0b10:
  ...
  private decodeLayer(bits: number): 1 | 2 | 3 | null {
    switch (bits) {
      case 0b11:
        return 1;
      case 0b10:
  ...
  private getSamplesPerFrame(version: 1 | 2 | 25, layer: 1 | 2 | 3): number {
    if (layer === 1) {
      return 384;
    }
